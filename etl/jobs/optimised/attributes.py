...
def with_slowly_changing_dimensions(df:DataFrame) -> DataFrame:
    return (df
        ...
        .withColumn("record_start_ts", lit("1970-01-01 00:00:00").cast("timestamp")) 
        .withColumn("record_end_ts", lit("2999-12-31 00:00:00").cast("timestamp")) 
        .withColumn("record_active_flag", lit(1).cast("smallint"))
        .withColumn("record_upd_ts", current_timestamp())
        .withColumn("record_insert_ts", current_timestamp())
    )